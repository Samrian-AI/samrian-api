...
# from core.config import get_settings # Removed
from .base_embedding_model import BaseEmbeddingModel # Adjusted import
from ..models.chunk import Chunk # Adjusted import
...
class ColpaliEmbeddingModel(BaseEmbeddingModel):
    def __init__(self,
                 ...
                 batch_size: int = 1 # Default to local-friendly batch size
                ):
        ...
        self.model = ColQwen2_5.from_pretrained(
            ...
            attn_implementation="flash_attention_2" if self.device == "cuda" else "eager", # Make configurable?
        ).eval()
        ...
        # self.settings = get_settings() # Removed
        # self.mode = self.settings.MODE # Removed, mode is implicit or controlled by batch_size
        self.batch_size = batch_size # Directly passed
    ...
    async def embed_for_ingestion(self, chunks: Union[Chunk, List[Chunk]]) -> List[np.ndarray]:
        ...
        # Removed self.mode from log message
        logger.info(
